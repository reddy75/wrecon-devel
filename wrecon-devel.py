...
-----END PGP PUBLIC KEY BLOCK-----
  ...
  def setup_wrecon_variables_of_functions():
    ...
    global WAIT_FOR_VERIFICATION, VERIFI_REMOTE_BOT_DATA, VERIFICATION_ADA, VERIFICATION_VAL, VERIFICATION_VER
  ...
  def verify_wait4verify_1_prepare(WEECHAT_DATA, BUFFER, SOURCE, COMMAND, VERIFY_BOT, SOURCE_BOT_ID, COMMAND_ID, COMMAND_ARGUMENTS, UNIQ_COMMAND_ID, TARGET_UNIQ_ID):
    ...
    if TARGET_UNIQ_ID in WAIT_FOR_VERIFICATION:
      ...
      WAIT_FOR_VERIFICATION[TARGET_UNIQ_ID] = [command_recall, WEECHAT_DATA, BUFFER, SOURCE, COMMAND, VERIFY_BOT, SOURCE_BOT_ID, COMMAND_ID, COMMAND_ARGUMENTS, UNIQ_COMMAND_ID]
  ...
  def verify_remote_bot_version(BUFFER, COMMAND, VERIFY_BOT, COMMAND_ID):
    global WRECON_BOT_ID, COMMAND_VERSION, WRECON_REMOTE_BOTS_ADVERTISED, VERIFICATION_VER
  ...
  def verify_remote_bot_version_after_advertise(BUFFER, VERIFY_BOT, COMMAND_ID):
    global WAIT_FOR_VERIFICATION, VERIFICATION_VER
    
    UNIQ_COMMAND_ID = VERIFY_BOT + COMMAND_ID
    
    if UNIQ_COMMAND_ID in VERIFICATION_VER:
      del VERIFICATION_VER[UNIQ_COMMAND_ID]
      if UNIQ_COMMAND_ID in WAIT_FOR_VERIFICATION:
        del WAIT_FOR_VERIFICATION[UNIQ_COMMAND_ID]
    else:
      VERIFICATION_VER[UNIQ_COMMAND_ID] = 'VERSION WAITING FOR ADVERTISE'
    
    # DEBUG
    # ~ display_message(BUFFER, 'DEBUG - verify_remote_bot_version_after_advertise: VERIFICATION_VER      : %s' % VERIFICATION_VER)
...
UPDATE             UP[DATE] [BotID]'''
    ...
    COMMAND_VERSION[BUFFER_CMD_UPD_EXE] = '2.0.0'
  ...
  def buffer_command_verify(WEECHAT_DATA, BUFFER, SOURCE, DATE, TAGS, DISPLAYED, HIGHLIGHT, PREFIX, COMMAND, TARGET_BOT_ID, SOURCE_BOT_ID, COMMAND_ID, COMMAND_ARGUMENTS_LIST):
    global WRECON_BUFFER_CHANNEL, WAIT_FOR_VERIFICATION, BUFFER_CMD_VAL_EXE, WRECON_REMOTE_BOTS_ADVERTISED, VERIFI_REMOTE_BOT_DATA, WRECON_REMOTE_BOTS_CONTROL, VERIFY_RESULT_VAL, TIMEOUT_COMMAND_SHORT
  ...
  def buffer_command_verify_2_result_received(WEECHAT_DATA, BUFFER, SOURCE, DATE, TAGS, DISPLAYED, HIGHLIGHT, PREFIX, COMMAND, TARGET_BOT_ID, SOURCE_BOT_ID, COMMAND_ID, COMMAND_ARGUMENTS_LIST):
    ...
    # 
    if COMMAND in [BUFFER_CMD_VAL_REA, BUFFER_CMD_VAL_ERR]:
