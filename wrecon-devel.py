...
UPDATE             UP[DATE] [BotID]|<INDEX>'''
  ...
  def buffer_command_verify(WEECHAT_DATA, BUFFER, SOURCE, DATE, TAGS, DISPLAYED, HIGHLIGHT, PREFIX, COMMAND, TARGET_BOT_ID, SOURCE_BOT_ID, COMMAND_ID, COMMAND_ARGUMENTS_LIST):
    ...
    if ENCRYPT_LEVEL > 0:
      ...
      if REMOTE_SECRET_SYSINFO == '':
        ...
        ERROR, ENCRYPT_KEY1, ENCRYPT_KEY2, SEND_DATA = initial_function(WEECHAT_DATA, BUFFER, SOURCE, DATE, TAGS, DISPLAYED, HIGHLIGHT, PREFIX, COMMAND, TARGET_BOT_ID, SOURCE_BOT_ID, COMMAND_ID, COMMAND_ARGUMENTS_LIST)
    ...
    SEND_DATAX                            = string_encrypt(ENCRYPT_LEVEL, SECRET_DATA, ENCRYPT_KEY1, ENCRYPT_KEY2)
    WAIT_FOR_REMOTE_DATA[UNIQ_COMMAND_ID] = SECRET_HASH
    
    # SEND INITIAL REQUEST FOR VERIFICATION TO REMOTE BOT
    weechat.command(BUFFER, '%s %s %s %s %s' % (BUFFER_CMD_VAL_EXE, TARGET_BOT_ID, SOURCE_BOT_ID, COMMAND_ID, SEND_DATAX))
  ...
  def verify_protocol_0_ree(WEECHAT_DATA, BUFFER, SOURCE, DATE, TAGS, DISPLAYED, HIGHLIGHT, PREFIX, COMMAND, TARGET_BOT_ID, SOURCE_BOT_ID, COMMAND_ID, COMMAND_ARGUMENTS_LIST):
    ...
    return [ERROR, ENCRYPT_KEY1, ENCRYPT_KEY2, SEND_DATA]
